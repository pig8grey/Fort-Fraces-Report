...
class CNPlotter():
    ...
    def __init__(self,mytime):
        ...
        self.vfiles=['/home/jeff/cndata/NBTM/NBTM_S5_{}_{}_{}.mseed'.format(
                    myyear,
                    mymonth,
                    myday,),
          '/home/jeff/cndata/SBTM/SBTM_S5_{}_{}_{}.mseed'.format(
                    myyear,
                    mymonth,
                    myday,),   
        ...
        self.vfiles2=['/home/jeff/cndata/NBTM/NBTM_S5_{}_{}_{}.mseed'.format(
                    myyear2,
                    mymonth2,
                    myday2,),
          '/home/jeff/cndata/SBTM/SBTM_S5_{}_{}_{}.mseed'.format(
        ...
        self.stfiles=[
            ...
            '/home/jeff/cndata/080/080_S5_{}_{}_{}.mseed'.format(
                      myyear,
                      mymonth,
                      myday,),  
            '/home/jeff/cndata/082/082_S5_{}_{}_{}.mseed'.format(
                    myyear,
                    mymonth,
                    myday,),
    ...
    def plotSt2(self,st,debug=False):
        ...
        if st:
            ...
            fig.savefig(f'/home/jeff/ffon/web/export/{myyear}_{mymonth}_{myday}/{sensor}-{plotname}.png')
            plt.clf()
            plt.close(fig)
            gc.collect()
            return
...
def plotStrain(st,plottype='',env=None):
    if st:
        ...
        plt.subplots_adjust(hspace=0, wspace=0)  # Adjust the right margin as needed
        plt.tight_layout()  # Increase padding between subplots
        fig.savefig(f'/home/jeff/ffon/web/export/{myyear}_{mymonth}_{myday}/{sensor}-{plotname}_{plottype}.png')
        plt.clf()
        plt.close(fig)
        gc.collect()  
    return
...
with Pool(processes=4) as pool:
    evtMax=pool.map(manualEvtst,cn.stst)
...
with Pool(processes=4) as pool:
    cn.disst=pool.map(readStream,zip(cn.files,cn.files2))
...
#     cn.disst[1][i].data=OriginalCN(cn.disst[1][i].data, np.zeros(8))[-1]
...
#     cn.disst=pool.map(getEnvelope,[[x, list(zip(mydate,evtend)).copy()] for x in cn.disst])




# with Pool(processes=2) as pool:
#     sz,nz=pool.map(getEnvelope2,[[x, list(zip(mydate,evtend)).copy(),pfs] for x in (sz,nz)])

# for i,st in enumerate(cn.disst): cn.disst[i]=cn.disst[i].select(component="[EN12]")
# cn.disst[0]+=nz
# cn.disst[1]+=sz
...
# for i,v in enumerate(cn.disst[0]):
#     if cn.disst[0][i].stats.channel=='BSZ':cn.disst[0][i]=nz[0]
#     if cn.disst[1][i].stats.channel=='BSZ':cn.disst[1][i]=sz[0]
...
# evtMax=plotEvtst(cn.stst[1])
# raise ('error')    
for i,row in enumerate(evtMax[0]):
    if i==0: mode='w'
    else:mode='a'
    writeStraincsv(evtMax[0][i][0],f'/home/jeff/ffon/strain/080_{myyear}-{mymonth}-{myday}_logs.csv',mode)

for i,row in enumerate(evtMax[1]):
    if i==0: mode='w'
    else:mode='a'
    writeStraincsv(evtMax[1][i][0],f'/home/jeff/ffon/strain/082_{myyear}-{mymonth}-{myday}_logs.csv',mode)


# for i,row in enumerate(evtMax):
#     if i==0: mode='w'
#     else:mode='a'

#     writeStraincsv(evtMax[i][0],f'/home/jeff/ffon/strain/082_{myyear}-{mymonth}-{myday}_logs.csv',mode)
    

# for i,row in enumerate(evtMax):
#     if i==0: mode='w'
#     else:mode='a'
#     writeStraincsv(evtMax[i][0],f'/home/jeff/ffon/strain/080_{myyear}-{mymonth}-{myday}_logs.csv',mode)
